 ...
 * scenario set (AC1 – AC2) with hearing date 20/05/2026.
 *
 ...
 *              (YRC2 / YRC1 / YRC3)</li>
 *   <li>AC3 — Multiple distinct error types shown simultaneously across defendants</li>
 *   <li>AC4 — Multiple errors shown above the same result or across separate offences</li>
 * </ul>
 */
class YroEndDateValidationIntegrationTest extends IntegrationTestBase {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // AC3 — Multiple Distinct Error Types Across Multiple Defendants
    // ═══════════════════════════════════════════════════════════════════════

    @Nested
    @DisplayName("AC3 — Multiple error types shown simultaneously")
    class Ac3MultipleErrorTypes {

        @Test
        @DisplayName("Scenario 9 — Three defendants; AC1 error, AC2a error, one valid")
        void three_defendants_two_distinct_error_types_one_valid_shows_all_errors()
                throws Exception {
            // d1: YROEW endDate 2026-05-19 (past) → AC1
            // d2: YROEW endDate 2026-10-01, YRC2 end 2026-11-01 → AC2a
            // d3: YROEW endDate 2027-06-01 → valid
            String request = """
                    {
                      "hearingId": "h-sc9",
                      "hearingDay": "2026-05-20",
                      ...
                      "resultLines": [
                        ...
                        {"resultLineId": "rl-d2-yrc2", "shortCode": "YRC2", "category": "I", "label": "Curfew",
                         "defendantId": "d2", "offenceId": "off2",
                         "prompts": [{"promptRef": "endDate", "promptValue": "2026-11-01"}]},
                        {"resultLineId": "rl-d3", "shortCode": "YROEW", "category": "F", "label": "YRO",
            ...
            mockMvc.perform(post(VALIDATE_URL)
                    ...
                    .andExpect(jsonPath(DR_YRO_ERRORS, hasSize(2)))
                    .andExpect(jsonPath("$.errors.validationIssues[*].affectedOffences[0].message",
                            containsInAnyOrder(MSG_AC1, MSG_YRC2)))
                    .andExpect(jsonPath("$.errors.errorMessages", hasSize(2)))
                    .andExpect(jsonPath("$.errors.errorMessages",
                            containsInAnyOrder(
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // AC4 — Error Display Above Each Result (multiple errors per offence / per defendant)
    // ═══════════════════════════════════════════════════════════════════════

    @Nested
    @DisplayName("AC4 — Errors shown above each result")
    class Ac4ErrorAboveResult {

        @Test
        @DisplayName("Scenario 10 — Single offence with both AC1 and AC2a violations simultaneously")
        void single_offence_ac1_and_ac2a_both_fire_simultaneously() throws Exception {
            // YROEW end 2026-05-19 (= day before hearing → AC1)
            ...
            mockMvc.perform(post(VALIDATE_URL)
                    ...
                    .andExpect(jsonPath("$.errors.errorMessages",
                            containsInAnyOrder(
                                    ERR_MSG_BASE_AC1 + "Laura Parker.",
                                    ERR_MSG_BASE_YRC2 + "Laura Parker.")));
        }

        @Test
        @DisplayName("Scenario 11 — Two offences, each with a different error type (AC1/AC2a)")
        void two_offences_each_with_a_different_error_type_produces_two_independent_errors()
                throws Exception {
