...
class CommunityOrderEndDatePreprocessorTest {
    ...
    @Nested
    @DisplayName("DUR-CUR — CUR end date does not match Start date + Curfew period - 1")
    class DurCur {
        ...
        void curfewPeriod_with_unrecognized_unit_should_skip_gracefully_no_violation() {
            // Only Days/Weeks/Months are confirmed real-world units; anything else falls back
            // to the existing WARN-and-skip behaviour rather than guessing a conversion.
            ResultLineDto cur = requirementLineWithPrompts("rl-cur", "CUR", "d1", "off1", Map.of(
                    "startDate", "2026-04-01",
        ...
        @Test
        void curfewPeriod_near_long_max_value_days_should_skip_gracefully_no_violation() {
            // Long.parseLong succeeds for a value this close to Long.MAX_VALUE (no
            // NumberFormatException), but startDate.plusDays(...) then overflows internally via
            // Math.addExact, throwing ArithmeticException rather than DateTimeException — both
            // must be caught and skipped, not thrown uncaught out of preprocess().
            ResultLineDto cur = requirementLineWithPrompts("rl-cur", "CUR", "d1", "off1", Map.of(
                    "startDate", "2026-04-01",
                    "curfewPeriod", "9223372036854775807 Days",
                    "endDate", "2026-06-10"));
        ...
        void curfewPeriod_with_months_suffix_should_parse_and_detect_violation() {
            // startDate 2025-12-01, curfewPeriod "1 Months", endDate 2026-02-02. Expected =
            // 2025-12-01 + 1 month - 1 day = 2025-12-31, so the recorded endDate is a genuine
            // mismatch.
            ResultLineDto cur = requirementLineWithPrompts("rl-cur", "CUR", "d1", "off1", Map.of(
    ...
    @Nested
    @DisplayName("DUR-CURE — CURE end date of tagging does not match duration")
    class DurCure {
    ...
    @Nested
    @DisplayName("DUR-AAR — AAR until date does not match hearing date + days - 1")
    class DurAar {
    ...
    @Nested
    @DisplayName("Cross-cutting: CURA exclusion and multi-defendant isolation")
    class DurationMismatchCrossCutting {
        ...
        void cura_requirement_with_duration_shaped_prompts_never_produces_any_duration_mismatch() {
            ...
            // mismatch if routed through the CUR duration check — but CURA is not one of the
            // duration-mismatch target requirement types (only CUR, CURE, AAR), so it must be a
            // no-op for all three duration-mismatch counters, even though AC2c (order-vs-CURA)
            ...
            assertThat(ctx.curaViolationCount()).isEqualTo(1L);
            // But none of the duration-mismatch counters are touched by CURA.
