 ...
 * DR-COEW-005 rule. Carries violation counts (exposed to CEL conditions) and the sets of
 * offence ids that triggered each violation (used by the message template resolver to
 * scope inline errors).
 *
 * <p>Also carries the requirement duration-mismatch counts/offence-id sets (CUR, CURE, AAR —
...
public record CommunityOrderContext(
    ...
    /**
     * Returns the four AC2 violation counts plus the three duration-mismatch counts as the CEL
     * variable map.
     *
