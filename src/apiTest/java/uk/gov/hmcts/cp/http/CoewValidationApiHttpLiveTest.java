 ...
 *   <li>AC3: UPWR with order duration shorter than 12 calendar months</li>
 *   <li>AC3 boundary: order duration exactly 12 calendar months (valid)</li>
 ...
 *   <li>Mixed AC2 + AC3 failures: {@code isValid=false}, correct error summary messages</li>
 * </ul>
 */
class CoewValidationApiHttpLiveTest {
    ...
    /**
     * Covers AC3: a COEW order with an UPWR requirement whose duration is shorter than 12 calendar
     * months from the hearing date, producing a blocking ERROR.
     */
    @Test
    void ac3_upwr_order_under_12_months_should_produce_error() throws Exception {
        final String body = """
                {
                  "hearingId": "h-coew-ac3",
                  "hearingDay": "2026-05-14",
                  "courtType": "MAGISTRATES",
                  "resultLines": [
                    {"resultLineId": "rl-order", "shortCode": "COEW", "category": "F",
                    ...
                    {"resultLineId": "rl-upwr", "shortCode": "UPWR", "category": "I",
                     "label": "Requirement", "defendantId": "d1", "offenceId": "off1"}
                  ],
                  "defendants": [{"defendantId": "d1", "firstName": "John", "lastName": "Smith"}],
                  "offences": [{"offenceId": "off1", "offenceCode": "TH68001",
        ...
        assertThat(issues.get(0).get("affectedOffences").get(0).get("offenceId").asText())
                .isEqualTo("off1");
        assertThat(json.get(ERRORS).get(ERROR_MESSAGES).get(0).asText())
                .isEqualTo(ERR_MSG_BASE_UPWR + "John Smith.");
    }

    /**
     * Covers the AC3 boundary: hearingDay 14/05/2026, order end 14/05/2027 is exactly 12 calendar
     * months so the order is valid and no error should be produced.
     */
    @Test
    void ac3_upwr_order_exactly_12_months_should_be_valid() throws Exception {
        final String body = """
                {
                  "hearingId": "h-coew-ac3-boundary",
                  "hearingDay": "2026-05-14",
                  "courtType": "MAGISTRATES",
                  "resultLines": [
                    {"resultLineId": "rl-order", "shortCode": "COS", "category": "F",
                     "label": "Community order", "defendantId": "d1", "offenceId": "off1",
                     "prompts": [{"promptRef": "endDate", "promptValue": "2027-05-14"}]},
                    {"resultLineId": "rl-upwr", "shortCode": "UPWR", "category": "I",
                     "label": "Requirement", "defendantId": "d1", "offenceId": "off1"}
                  ],
                  "defendants": [{"defendantId": "d1", "firstName": "Boundary", "lastName": "Valid"}],
                  "offences": [{"offenceId": "off1", "offenceCode": "TH68001",
                                "offenceTitle": "Theft", "orderIndex": 1}]
                }
                """;
    ...
    /**
     * Covers mixed AC2 and AC3 failures across two defendants with a third valid defendant:
     * {@code isValid} must be false, and both error summary messages must carry the defendant name.
     */
    @Test
    void mixed_ac2_and_ac3_failures_set_isValid_false_with_correct_summary_messages()
            throws Exception {
        final String body = """
                {
                  "hearingId": "h-coew-mixed",
                  "hearingDay": "2026-05-14",
                  "courtType": "MAGISTRATES",
                  "resultLines": [
                    {"resultLineId": "rl-d1-order", "shortCode": "COEW", "category": "F",
                    ...
                    {"resultLineId": "rl-d2-order", "shortCode": "COEW", "category": "F",
                     "label": "Community order", "defendantId": "d2", "offenceId": "off2",
                     "prompts": [{"promptRef": "endDate", "promptValue": "2027-04-13"}]},
                    {"resultLineId": "rl-d2-upwr", "shortCode": "UPWR", "category": "I",
                     "label": "Requirement", "defendantId": "d2", "offenceId": "off2"}
                  ],
                  "defendants": [
        ...
        assertThat(errorMessages)
                .contains(ERR_MSG_BASE_CUR + "CUR Violator.")
                .contains(ERR_MSG_BASE_UPWR + "UPWR Violator.");
    }
