...
/**
 * Live HTTP coverage for DR-AGE-007 (imprisonment result age restriction) against a running
...
class AgeRestrictedImprisonmentApiHttpLiveTest {
     ...
     * not raise DR-AGE-007, must not error the request, and must not stop a different rule
     * (DR-SENT-001) from evaluating and reporting on the same payload.
     */
    @Test
     ...
     * treated as 21 or over and must not raise DR-AGE-007.
     */
    @Test
    ...
    void multipleQualifyingOffencesSameDefendant_shouldNameDefendantOnce() throws Exception {
        // rl2 carries isConcurrent=true purely so DR-SENT-001's custodial preprocessor (which
        // also treats IMP/EXTIVS as custodial short codes) sees relationship info on both
        // offences and stays silent — this test isolates DR-AGE-007's own dedup behaviour.
        final String body = """
                {
                  "hearingId": "h1",
                  "hearingDay": "2026-07-20",
