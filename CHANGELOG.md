...
- **DR-CONV-006** — No conviction on sentenced offence warning: warns when an offence has
  ...
  `NoConvictionPreprocessor` (DD-43039).
...
- All shipped validation rules (`DR-SENT-001`, `DR-DISQ-002`, `DR-CTL-003`, `DR-YRO-004`,
  `DR-COEW-005`, `DR-CONV-006`) now seed `enabled=true` by default in fresh databases, matching
  the manually-enabled steady state already in place on SIT/PRD.
...
- Dead per-rule enable/disable plumbing in the `apiTest` and integration-test suites
  ...
  support) for `DR-CTL-003`, `DR-YRO-004`, `DR-DISQ-002`, `DR-COEW-005`, and `DR-CONV-006` —
  now unnecessary since every rule seeds enabled by default. Per-rule override/severity-
