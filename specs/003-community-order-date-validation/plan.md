...
Implement the `DR-COEW-001` validation rule that detects two categories of community order date error at "Save and continue":

- **AC2** — A community order's end date is earlier than the end date of any attached requirement (CUR, CURE, CURA, or AAR).
- **AC3** — A community order containing an unpaid work requirement (UPWR) has an end date less than 12 months from the hearing date.
...
**Scale/Scope**: One new YAML rule, one new preprocessor, one new context record, six new fields on an existing class
...
| **VI — Severity Ceiling** | ✅ PASS | All five conditions set `severity: ERROR`; ceiling can lower to WARNING at runtime but never promotes |
...
Key resolved items:
- `hearingDay: LocalDate` confirmed on `DraftValidationRequest` (no upstream change needed)
...
- UPWR boundary: `orderEndDate.isBefore(hearingDay.plusMonths(12).minusDays(1))`
...
|------|--------|----------|
| `PreprocessingDefinition` | +6 `List<String>` fields | `cel/PreprocessingDefinition.java` |
...
| `DR-COEW-001.yaml` | NEW rule, priority 4000, 5 conditions | `resources/rules/DR-COEW-001.yaml` |

### 1b. Interface contracts

No new API surface. The existing `POST /api/validation/validate` endpoint is unchanged. New `ValidationIssue` entries of type ERROR will appear in the `errors` list of `DraftValidationResponse` when AC2 or AC3 violations are detected — this is an additive, backward-compatible change.
...
Add the 6 new `List<String>` fields. This is a pure data change; existing tests must still pass.
...
1. Write `CommunityOrderEndDatePreprocessorTest` covering:
   ...
   - AC3: UPWR + order < 12 months → `upwrViolationCount = 1`
   - Boundary: requirement date equal to order date → no violation (AC2)
   - Boundary: order end = `hearingDay + 12m - 1d` → no violation (AC3)
...
1. Write `CommunityOrderEndDateRuleIntegrationTest` extending `IntegrationTestBase`:
   - Scenarios 6–18 from the spec (AC2 and AC3)
