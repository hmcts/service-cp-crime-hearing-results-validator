...
> **AC3 removed**: The UPWR 12-month minimum order duration check (AC3) has been removed from scope. `unpaidWorkShortCodes` has been removed from `PreprocessingDefinition`, the `AC3` condition has been removed from `DR-COEW-001.yaml`, and `upwrViolationCount`/`upwrViolationOffenceIds` have been removed from `CommunityOrderContext`.
...
1. Write `CommunityOrderEndDatePreprocessorTest` covering:
   ...
   - Boundary: requirement date equal to order date → no violation
