...
**Precedent**: `specs/002-extended-test-disq-warning` hit the identical situation for the
...
## R2 — Rule id / category: `DR-AGE-007`, not `DR-SENT-003`

**Decision**: Category `AGE`, id `DR-AGE-007`.
...
`specs/002-extended-test-disq-warning/research.md` (R3 in that document) established the same
...
- *Extend `DefendantContext` with `isUnder21`/`hasImprisonmentResult` fields* — rejected: couples
  ...
  consecutive domain, and would force `DR-SENT-002` and `DR-AGE-007` to share
