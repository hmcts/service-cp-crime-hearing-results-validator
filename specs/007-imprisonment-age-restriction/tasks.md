...
**Input**: Design documents from `/specs/007-imprisonment-age-restriction/`
...
- [X] T008 [P] Write failing integration test
      ...
      existing rule (e.g. `DR-CTL-001` or `DR-SENT-002`). Assert: no `DR-AGE-007` issue is
      raised; the other rule's issue IS present in the response; the HTTP response is a normal
      200 (no error surfaced to the caller); `rulesEvaluated` still lists `DR-AGE-007` (proving
      the rule ran to completion rather than being caught and skipped by
      `DefaultValidationService`'s per-rule exception handler).

**Checkpoint**: `gradle build` passes; `DR-AGE-007` loads at startup via
...
- [X] T009 [US1] Write failing integration test
      ...
      issue with `ruleId="DR-AGE-007"`. (Same file as T008 — sequenced after it, not parallel.)
...
`dateOfBirth` changed so the defendant is 21+ on `hearingDay`. Assert no `DR-AGE-007` issue in the
second response.
...
- [X] T015 [US3] In `AgeRestrictedImprisonmentRuleIT.java`, add
      ...
      21+ years before `hearingDay`, asserting no `DR-AGE-007` issue in the second response.
...
- [X] T020 Run this feature through the mandatory build loop
      ...
      **Done**: `code-reviewer` → PASS (0 HIGH, 1 MEDIUM, 1 LOW). `spec-validator` → COMPLIANT
      ...
      `AC2` → `AC1` in `DR-AGE-007.yaml` to match the `AC1`-first convention used by
...
**Do not** add a new per-rule severity-ceiling/override integration test for `DR-AGE-007`. That
...
only if evaluating `DR-AGE-007` surfaces a genuine gap in the shared override mechanism itself.
...
- [X] T022 [P] Add live API test coverage in
      ...
      `DR-AGE-007` ships `enabled=true`, same as `DR-SENT-002`). Covers the same 8 scenarios as
...
Task: "Add src/main/resources/rules/DR-AGE-007.yaml"
