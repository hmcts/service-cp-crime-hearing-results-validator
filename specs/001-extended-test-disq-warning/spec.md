...
- **Per-defendant evaluation** — when a hearing contains multiple defendants, the rule produces at most one warning per defendant–offence pair, matching the per-defendant grouping that the existing custodial rule uses.
