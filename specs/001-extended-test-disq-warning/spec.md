...
- **Cross-repo coordination under DD-41656** — the contract change spans four repositories (validator API, validator service, share-time command handler, results-entry UI) but ships under a single Jira (no separate ticket per repo). Each repo carries its own `DD-41656`-prefixed branch. The branching question for `cpp-context-hearing` (whether DD-41656 piggybacks on the open `DD-41715-custodial-concurrent-consecutive-check` branch or gets its own branch off integration) is to be confirmed before implementation; a separate branch is the safer default unless DD-41715 is about to merge.
