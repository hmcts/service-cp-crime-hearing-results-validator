...
- No `[NEEDS CLARIFICATION]` markers were inserted: AC1, AC1A, and the "Logic" section in the source ticket are unambiguous on the five offence codes, the nine excluded final-result codes, the two suppressing disqualification codes, the exact warning text, and the non-blocking severity. Multi-defendant behaviour, case sensitivity, and rule packaging are covered in **Assumptions** with reasonable defaults consistent with the existing DR-SENT-001 rule.
