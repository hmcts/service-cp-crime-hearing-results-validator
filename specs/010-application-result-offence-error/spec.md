...
A user records application-only results against more than one offence — for the same defendant, across several defendants, or more than one application-only result against a single offence. When they select "Save and continue", the page-level error identifies every breaching result, the "This affects:" line lists every affected defendant (comma-separated where there is more than one), and an inline error appears for each breaching result below its offence. Every one of the errors must be resolved before sharing is possible.
...
1. **Given** application-only results are recorded against more than one offence for a single defendant, **When** validation is triggered, **Then** the page-level error identifies every breaching result, **And** an inline error is returned for each breaching result below its own offence. *(AC2)*
2. **Given** more than one application-only result is recorded against the same offence, **When** validation is triggered, **Then** a separate inline error is returned for each breaching result on that offence.
3. **Given** application-only results are recorded against offences belonging to more than one defendant, **When** validation is triggered, **Then** the "This affects:" line lists every affected defendant, comma-separated, in hearing display order.
4. **Given** several breaches exist and the user resolves some but not all of them, **When** validation is re-triggered, **Then** errors are still returned for the unresolved breaches **And** sharing remains blocked.
5. **Given** several breaches exist, **When** the user resolves all of them and validation is re-triggered, **Then** no errors are returned by this rule **And** sharing is no longer blocked by it.
...
- What happens when the same application-only result code appears on two different offences for the same defendant? → One inline error per breaching result occurrence (two inline errors); the defendant is named once in the "This affects:" list.
- What happens when the same application-only result code breaches on offences belonging to two different defendants? → An inline error per occurrence; both defendants are named in the "This affects:" list, comma-separated.
...
- **FR-004**: System MUST produce a page-level error message of the form "\<Application result label\> is an application result. It cannot be added to an offence. Remove it from the offence and add an application to the hearing", where \<Application result label\> is the human-readable result label (not the code) of the breaching result.
...
- **FR-006**: System MUST produce an inline error message of the form "Remove \<Application result label\> from this offence. It is an application result, so it can only be added to an application." for each breaching result, associated with the offence it was recorded against.
- **FR-007**: System MUST identify every breaching result in a single validation pass — it MUST NOT stop at the first breach, whether the breaches are on one offence, several offences for one defendant, or offences across several defendants.
- **FR-008**: System MUST report the affected offence and defendant for each issue so the consuming screen can position the inline error below the correct offence and above the correct result.
