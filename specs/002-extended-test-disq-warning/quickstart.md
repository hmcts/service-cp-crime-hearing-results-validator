...
The new rule is auto-discovered at startup by `ValidationRuleAutoConfiguration` from `src/main/resources/rules/DR-DISQ-002.yaml`. Look for this log line on startup:

```
INFO  ...config.ValidationRuleAutoConfiguration : Auto-discovered 2 validation rule(s) from classpath:rules/
```

If you see `1` instead of `2`, the YAML did not load — check the file is named `DR-DISQ-002.yaml` (the loader filters on the `DR-` prefix) and that YAML parsing did not throw on startup.
...
Expected: response contains exactly one `ValidationIssue` with `ruleId: "DR-DISQ-002"`, `severity: "WARNING"`, the exact AC1A message, and `affectedOffences: [{ "offenceId": "o-1", ... }]`.

### Suppressed by an excluded final result (on the F line)

Replace `resultLines[0].shortCode` with `"wdrn"` (keep `category: "F"`). Expected: zero `DR-DISQ-002` issues. (You may still see issues from other rules.) Note: under the new gate, the excluded short code must be on the **F-category** line — an excluded short code on an `A`/`I` line no longer suppresses.
...
Expected: zero `DR-DISQ-002` issues against offence `o-1`. DDOTE/DDOTEL on any category suppresses — by design (the rule's purpose is satisfied wherever the disqualification line sits).
...
Expected: zero `DR-DISQ-002` issues. The relevant offence has no F-category line, so the rule does not fire — even though `ADJN` is not in the excluded-final list. This is the canonical regression test for the 2026-04-28 revision (see SC-002 in spec.md).
...
Expected: two `DR-DISQ-002` issues, one per offence id.

### Disable via DB override

```sql
INSERT INTO validation_rule (id, enabled, severity, updated_at, updated_by)
VALUES ('DR-DISQ-002', false, NULL, now(), 'manual-test');
```

After ≤30 s (Caffeine TTL on `RuleOverrideService`), the smoke test from step 3.1 returns zero `DR-DISQ-002` issues without service restart.
...
# This rule
curl -s http://localhost:4550/api/validation/rules/DR-DISQ-002 | jq .
```
...
| Service starts but the rule is missing from `/api/validation/rules` | YAML file not on the classpath — check filename (`DR-DISQ-002.yaml`, exact case), or YAML parse error in startup logs. |
