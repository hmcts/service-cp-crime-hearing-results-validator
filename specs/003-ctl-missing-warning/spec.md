...
- **FR-004**: For each offence that has a trigger result and no existing CTL record, the system MUST check whether the current hearing includes a CTL result (one of the short codes `CTL`, `CCII`, `CCIIB`, `CCIILA`, `CCIITDH`, `CCIIYDA`, `CCQB`) against that offence. If one exists, no warning is produced for that offence.
...
- **2026-08-20**: Widened FR-004's CTL-result short code list from `CTL` alone to `CTL`, `CCII`, `CCIIB`, `CCIILA`, `CCIITDH`, `CCIIYDA`, `CCQB` — these additional codes are recognised outcomes that also record a Custody Time Limit in the current hearing and must bypass the warning identically to `CTL`. No new bypass condition was introduced; the existing CTL-result bypass (FR-004) was widened in place via the YAML-configured `ctlShortCodes` list.
