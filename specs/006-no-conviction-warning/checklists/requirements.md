...
- The excluded final short-code list (`wdrn`, `WDRNOFF`, `dism`, `dine`, `dini`, `disch`, `disc`, `ctrof`, `iremfile`) is carried over unchanged from the extended-test disqualification rule's equivalent list, per the user's explicit instruction to reuse existing logic where possible.
